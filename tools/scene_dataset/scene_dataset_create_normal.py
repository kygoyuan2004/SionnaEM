...
UE_Z = 1.5
BS_POS = np.array([0.7, -0.3, 1.5], dtype=np.float32)
L_KEEP = 32

DEFAULT_OUT_PATH = (
    Path(__file__).resolve().parent / "nf_rt_dataset_28g_32x32_1x1_k1024_CIR_floorwall_2000.npz"
)
...
def sample_rx_position(
    ...
    """Uniformly sample a UE position in an annulus around the BS."""
    if rng is None:
    ...
    x = center_pos[0] + r * np.cos(ang)
    y = center_pos[1] + r * np.sin(ang)
...
def generate_one_sample(
    ...
    ue_pos = sample_rx_position(BS_POS, R_MIN, R_MAX, z=UE_Z, rng=rng)
